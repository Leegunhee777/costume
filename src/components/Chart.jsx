...
const callApi1 = async () => {
  const response = await fetch('http://localhost:8001/category')
  const body = await response.json();
 // console.log(body); //consolelog찍어보면 res로 image경로가 제대로넘어오긴한다 근데 나오질않는다 시발!!!
  return body;
}
  const callApi2 = async () => {
    const response = await fetch('http://localhost:8001/category/detail')
    const body = await response.json();
   // console.log(body); //consolelog찍어보면 res로 image경로가 제대로넘어오긴한다 근데 나오질않는다 시발!!!
    return body;
  }
  ...
  const callApi3 = async () => {
    const response = await fetch('http://localhost:8001/month')
    const body = await response.json();
   // console.log(body); //consolelog찍어보면 res로 image경로가 제대로넘어오긴한다 근데 나오질않는다 시발!!!
    return body;
  }

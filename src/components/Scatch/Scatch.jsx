...
export default class Scatch extends React.Component {
    constructor(props) {
        ...
        this.deletecodilist2 = this.deletecodilist2.bind(this);
        this.deletecodilist3 = this.deletecodilist3.bind(this);
    }
   ...
   capturebutton(){  //이 함수는 내가 캔버스에 올린 전체 캡쳐와 캔버스위의 구성 object url을 보내는 함수임
    ...
    setTimeout(() => {
      ...
      var blob,i
        ...
        console.log(a);
      const url = 'http://172.16.101.144:8001/closet/';
      const formData = new FormData();
    
      formData.append('image',blob)
      formData.append('product',a);
    // formData.append('producturl',canvas.toJSON().objects)
  ...
  deletecodilist1(){  //////////////////////////////삭제버큰누르면 해당 props.basket.에 해당하는 제품을 basket에서 삭제시키고 나머지들로 구성된배열로 갱신시킴 ,그러다보니 props.basket[0]의영향을 받는 canvas캡쳐도갱신됨 이상하게되는부분이있다.
    ...
    this.props.updateUsercodilist(      
     ...
     deletecodilist2(){ 
      let a =this.props.basket.filter(c=> c !== this.props.basket[1])
      this.props.updateUsercodilist(         
       a 
      )
       // if(this.props.basket.length <6){ //props.basket[0]으로 물리적인제한을뒀으니.basket배열도 한정할필요가있음
       ...
       deletecodilist3(){ 
        let a =this.props.basket.filter(c=> c !== this.props.basket[2])
        this.props.updateUsercodilist(        
         a 
        )
         // if(this.props.basket.length <6){ //props.basket[0]으로 물리적인제한을뒀으니.basket배열도 한정할필요가있음
 ...
 /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  storebasket1(){ //버튼클릭하면 해당 상품 장바구니 DB로 연결
   ...
   fetch('http://172.16.101.113:3000/toolbarAdd/',{
    method: "POST",
        ...
        return(
          <div style={{
           ...
           border:'10px solid blue' ,
            ...
            <button onClick={this.capturebutton} >캔버스전체캡쳐뜨기와구성품보내기</button>
            ...
            <button id="delete">Delete selected image</button>
             ...
             <TableCell>   {this.props.basket[0]?   <button onClick={this.storebasket1}>장바구니담기</button>   :null}  </TableCell>
               <TableCell> {this.props.basket[0]?     <button onClick={this.deletecodilist1}>삭제</button>:null} </TableCell>
               ...
               <TableCell>{this.props.basket[1]? <button onClick={this.deletecodilist2}>삭제</button> :null} </TableCell>
               </TableRow>
               ...
               <TableCell>{this.props.basket[2]? <button onClick={this.deletecodilist3}>삭제</button> :null} </TableCell>
             </TableRow>
                 
                  
             <TableRow>
               <TableCell><img src={this.props.basket[3]}  width='80'/></TableCell>
               <TableCell>  {this.props.basket[3]?   <button>장바구니담기</button> :null} </TableCell>
               <TableCell>{this.props.basket[3]?<button>삭제</button>  :null} </TableCell>
             </TableRow>
             
               
             <TableRow>
               <TableCell><img src={this.props.basket[4]}  width='80'/></TableCell>
               <TableCell>  {this.props.basket[4]?   <button>장바구니담기</button>    :null} </TableCell>
               <TableCell>{this.props.basket[4]? <button>삭제</button>   :null} </TableCell>
             </TableRow>
             
             
                  <TableRow>
                    <TableCell><img src={this.props.basket[5]}  width='80'/></TableCell>
                    <TableCell>  {this.props.basket[5]?  <button>장바구니담기</button> :null}  </TableCell>
                    <TableCell> {this.props.basket[5]? <button>삭제</button>:null}   </TableCell>
                  </TableRow>
               
                   
                  <TableRow>
                    <TableCell><img src={this.props.basket[6]}  width='80'/></TableCell>
                    <TableCell>  {this.props.basket[6]?  <button>장바구니담기</button>    :null}</TableCell>
                    <TableCell> {this.props.basket[6]?  <button>삭제</button>  :null}  </TableCell>
                  </TableRow>
               
                  
                  <TableRow>
                    <TableCell><img src={this.props.basket[7]}  width='80'/></TableCell>
                    <TableCell> {this.props.basket[7]?  <button>장바구니담기</button>  :null} </TableCell>
                    <TableCell>{this.props.basket[7]? <button>삭제</button>  :null} </TableCell>
                  </TableRow>
                  
                 
                  <TableRow>
                    <TableCell><img src={this.props.basket[8]}  width='80'/></TableCell>
                    <TableCell>  {this.props.basket[8]?  <button>장바구니담기</button>   :null}  </TableCell>
                    <TableCell>  {this.props.basket[8]?  <button>삭제</button>  :null} </TableCell>
                  </TableRow>
  ...
  function gotocodi() // 단순히 캔버스위 objct 찍어보는수단
  {
    console.log(canvas._objects);   //캔버스 위의 object
